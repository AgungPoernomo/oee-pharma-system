...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 1. LOGIKA OEE ZONE F
    // ==========================================
    if (action === 'submit_reject_f' || action === 'update_reject_f') {
      const tableName = `oee_line${lineNum}_zonef`;
    ...
    // ==========================================
    // 2. LOGIKA DOWNTIME ZONE F
    // ==========================================
    else if (action === 'submit_downtime_f' || action === 'update_downtime_f') {
      const tableName = `downtime_line${lineNum}_zonef`;
    ...
    // Proses ini berjalan di belakang layar
    axios.post(process.env.GAS_URL, {
      action: action,
      data: backupData,
      user: user
    }).catch(err => console.error(`[Backup GAS Line ${lineNum} Zone F Gagal]`, err.message));
