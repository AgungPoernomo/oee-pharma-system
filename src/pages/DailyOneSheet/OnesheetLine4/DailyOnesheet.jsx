...
const calculateZoneMetrics = (volume) => {
    let speed = 12000;
    let teoriBatch = 21730;

    if (volume === '100 ML') { speed = 11500; teoriBatch = 56880; }
    else if (volume === '250 ML') { speed = 12000; teoriBatch = 21509; }
    else if (volume === '500 ML') { speed = 12000; teoriBatch = 21730; }
...
const DailyOnesheet = () => {
    const { user } = useAuth();
    const line4User = useMemo(() => ({ ...(user || {}), line: "4", plant: "1" }), [user]);
    ...
    const executeSearch = async () => {
        ...
        try {
            const res = await fetchOnesheetData(inputDate, line4User);
