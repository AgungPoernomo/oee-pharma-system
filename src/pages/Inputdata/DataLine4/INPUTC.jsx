...
const DT_COLS_META = [
  ...
  { title: 'Proses', width: 125, type: 'select', options: ['Blowing', 'Filling', 'Mixing', 'Autoclave', 'Preform'] },
...
export default function InputC() {
  ...
  const handleDownloadExcelOEE = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetchAllRejectC(line4User);
      if (res?.status === 'success' && Array.isArray(res.data)) {
        ...
        XLSX.writeFile(wb, `Semua_Data_OEE_Line4_ZoneC_${today}.xlsx`);
  ...
  const handleDownloadExcelDT = useCallback(async () => {
    ...
    try {
      const line4User = { ...(user || {}), line: "4" };
      const res = await fetchAllDowntimeC(line4User);
      if (res?.status === 'success' && Array.isArray(res.data)) {
        ...
        const today = new Date().toISOString().split('T')[0];
        XLSX.writeFile(wb, `Semua_Data_Downtime_Line4_ZoneC_${today}.xlsx`);
