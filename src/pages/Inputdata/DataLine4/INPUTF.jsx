...
const TEORI_BATCH = { "100 ML": 56880, "250 ML": 21509, "500 ML": 21730, "1000 ML": 60194 };
const TEORI_YIELD = 21730;
...
export default function InputF() {
  ...
  const handleDownloadExcelOEE = useCallback(async () => {
    ...
    try {
      const line4User = { ...(user || {}), line: "4" };
      const res = await fetchAllRejectF(line4User);
      if (res?.status === 'success' && Array.isArray(res.data)) {
        ...
        XLSX.writeFile(wb, `Semua_Data_OEE_Line4_ZoneF_${today}.xlsx`);
  ...
  const handleDownloadExcelDT = useCallback(async () => {
    ...
    try {
      const line4User = { ...(user || {}), line: "4" };
      const res = await fetchAllDowntimeF(line4User);
      if (res?.status === 'success' && Array.isArray(res.data)) {
        ...
        const today = new Date().toISOString().split('T')[0];
        XLSX.writeFile(wb, `Semua_Data_Downtime_Line4_ZoneF_${today}.xlsx`);
