...
export default function InputF() {
  ...
  const triggerAutosaveOEE = useCallback((rIdx, rowData) => {
    // Autosave dimatikan sesuai permintaan
  }, [line1User]);
  ...
  const handleActionOEE = useCallback(async (actionType, targetRowIdx) => {
    ...
    try {
      ...
      if (actionType === 'save' && !gas_id) {
        gas_id = `GAS_OEE_F_L1_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        gasOeeIds.current[targetRowIdx] = gas_id;
        localStorage.setItem(LS_GAS_IDS_OEE, JSON.stringify(gasOeeIds.current));
      }

      if ((actionType === 'save' || actionType === 'update') && gas_id) {
        const gasAction = actionType === 'save' ? 'direct_append_f' : 'direct_update_f';
        ...
        promises.push(fetch('https://script.google.com/macros/s/AKfycbyO_Rh0wzfpLPO83RuPh-mSHfeCmHMbfW1WkazHKbGmUT1RobjNTUTwrmsEhxv5lhit/exec', {
          ...
          body: JSON.stringify({
            action: gasAction,
            user: line1User,
            data: { gas_id: gas_id, rowData: gasRowData }
          })
  ...
  const handleActionDT = useCallback(async (actionType, targetRowIdx) => {
    ...
    try {
      ...
      if (actionType === 'save' && !gas_id) {
        gas_id = `GAS_DT_F_L1_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        gasDtIds.current[targetRowIdx] = gas_id;
        localStorage.setItem(LS_GAS_IDS_DT, JSON.stringify(gasDtIds.current));
      }

      if ((actionType === 'save' || actionType === 'update') && gas_id) {
        const gasAction = actionType === 'save' ? 'direct_append_dt_f' : 'direct_update_dt_f';
        dtPromises.push(fetch('https://script.google.com/macros/s/AKfycbyO_Rh0wzfpLPO83RuPh-mSHfeCmHMbfW1WkazHKbGmUT1RobjNTUTwrmsEhxv5lhit/exec', {
          ...
          body: JSON.stringify({
            action: gasAction,
            user: line1User,
            data: { gas_id: gas_id, rowData: [...rowData] }
          })
        }).then(r => r.json()).catch(() => ({ status: 'error' })));
      }
  ...
  const handleDownloadExcelOEE = useCallback(async () => {
    ...
    try {
      const res = await fetchAllRejectF(line1User);
      if (res?.status === 'success' && Array.isArray(res.data)) {
        const headers = OEE_COLS_META.map(col => col.title);
        const exportRows = res.data.map((row) => {
          const r = [
            row.no_batch ?? '', row.lot_no ?? '', parseToYMD(row.tanggal), row.shift ?? '', row.group ?? '', row.volume_botol ?? '',
            row.steril_in ?? '', row.steril_bocor ?? '', row.steril_h_patah_ring ?? '', row.steril_h_patah_lidah ?? '',
            row.steril_h_patah_leleh ?? '', row.steril_no_hanger ?? '', row.steril_rej_total ?? '', row.steril_sample ?? '',
            row.steril_out ?? '', row.vi_start ?? '', row.vi_end ?? '', row.vi_sub ?? '', '', row.vi_partikel ?? '',
            row.vi_kotik ?? '', row.vi_rej_total ?? '', row.vi_hasil_baik ?? '', row.vi_qc ?? '', row.vi_tf_packing ?? '',
            row.pack_reject ?? '', row.pack_hasil_baik ?? '', row.pack_s_qc ?? '', row.pack_s_others ?? '', row.pack_fg ?? '',
            row.pack_utuh ?? 'Y', row.pack_jml_batch ?? '', '', '', row.av_sh ?? '', row.av_sm ?? '',
            row.av_eh ?? '', row.av_em ?? '', row.av_sub ?? '', row.total_avail_shift ?? '', row.run_sh ?? '',
            row.run_sm ?? '', row.run_eh ?? '', row.run_em ?? '', row.run_sub ?? '', row.clear_sh ?? '',
            row.clear_sm ?? '', row.clear_eh ?? '', row.clear_em ?? '', row.clear_sub ?? '', row.process_total ?? ''
          ];
        ...
        XLSX.writeFile(wb, `Semua_Data_OEE_Line1_ZoneF_${today}.xlsx`);
  ...
  const handleDownloadExcelDT = useCallback(async () => {
    ...
    try {
      const res = await fetchAllDowntimeF(line1User);
      if (res?.status === 'success' && Array.isArray(res.data)) {
        ...
        const today = new Date().toISOString().split('T')[0];
        XLSX.writeFile(wb, `Semua_Data_Downtime_Line1_ZoneF_${today}.xlsx`);
