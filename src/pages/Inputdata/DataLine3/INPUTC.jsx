...
const getEmptyOEE = () => { const arr = Array(55).fill(''); arr[C.UTUH] = ''; return arr; };
const getEmptyDT = () => { const arr = Array(14).fill(''); arr[DC.TYPE] = ''; return arr; };
...
export default function InputC() {
  ...
  const oeeIds = useRef(getCachedIds('L3_C_IDS_OEE')); 
  const dtIds = useRef(getCachedIds('L3_C_IDS_DT'));  
  ...
  const triggerAutosaveOEE = (rIdx, sheet) => {
    ...
    oeeTimers.current[rIdx] = setTimeout(async () => {
      ...
      const payloadData = {
        ...
        volume_botol: rowData[C.VOL_BOTOL], cnt_start: rowData[C.CNT_START], cnt_end: rowData[C.CNT_END],
        cnt_sub: rowData[C.CNT_SUB], utuh: rowData[C.UTUH], jml_batch: rowData[C.JML_BATCH],
        ...
        trf_st: rowData[C.TRF_TO_ST], pre_in: rowData[C.INPUT_STERIL], pre_bocor: rowData[C.REJ_BOCOR],
        pre_nocap: rowData[C.REJ_TANPA_CAP], pre_vol: rowData[C.REJ_VOL], pre_thermo: rowData[C.REJ_THERMO],
        pre_lain: rowData[C.REJ_LAINLAIN], pre_rej_total: rowData[C.TOTAL_REJ_BS], pre_out: rowData[C.OUTPUT_CHAMBER],
        av_sh: rowData[C.AT_SH], av_sm: rowData[C.AT_SM], av_eh: rowData[C.AT_EH], av_em: rowData[C.AT_EM],
        av_sub: rowData[C.AT_SUB], total_avail_shift: rowData[C.AT_TOTAL], run_sh: rowData[C.RT_SH],
        run_sm: rowData[C.RT_SM], run_eh: rowData[C.RT_EH], run_em: rowData[C.RT_EM], run_sub: rowData[C.RT_SUB],
      ...
      if (res.status === 'success' && res.original_id) {
        ...
        localStorage.setItem('L3_C_IDS_OEE', JSON.stringify(oeeIds.current));
      }
    }, 1000);
  };
  ...
  const triggerAutosaveDT = (rIdx, sheet) => {
    ...
    dtTimers.current[rIdx] = setTimeout(async () => {
      ...
      if (res.status === 'success' && res.original_id) {
        ...
        localStorage.setItem('L3_C_IDS_DT', JSON.stringify(dtIds.current));
      }
    }, 1000);
  };
  ...
  const loadDataServer = useCallback(async () => {
    ...
    try {
      const [resOEE, resDT] = await Promise.all([
        fetchTodayRejectC({ ...user, line: "3" }),
        fetchTodayDowntimeC({ ...user, line: "3" }),
      ]);
      ...
      localStorage.setItem('L3_C_DATA_OEE', JSON.stringify(finalOEE));
      localStorage.setItem('L3_C_DATA_DT', JSON.stringify(finalDT));
      localStorage.setItem('L3_C_IDS_OEE', JSON.stringify(oeeIds.current));
      localStorage.setItem('L3_C_IDS_DT', JSON.stringify(dtIds.current));
  ...
  useEffect(() => {
    const initialOEE = getCachedData('L3_C_DATA_OEE', getEmptyOEE, 100);
    const initialDT  = getCachedData('L3_C_DATA_DT', getEmptyDT, 100);
    ...
    if (oeeTableRef.current) {
      ...
      oeeGrid.current = jspreadsheet(oeeTableRef.current, {
        worksheets: [{
          ...
          freezeColumns: 2, tableOverflow: true, tableWidth: '100%', tableHeight: '700px',
        }],
      ...
      dtGrid.current = jspreadsheet(dtTableRef.current, {
        worksheets: [{
          ...
          columns: [
            ...
            { 
              type: 'dropdown', title: 'Unit', width: 120, source: ALL_UNITS_C,
      ...
      <div className="max-w-full mx-auto">
        <div className="mb-4">
          <h1 className="text-2xl font-black tracking-wider uppercase text-emerald-800">
            OEE Line 3 — Zone C <span className="text-sm font-normal normal-case text-gray-500 ml-2">(Auto-Saving & Cached)</span>
          </h1>
        </div>
