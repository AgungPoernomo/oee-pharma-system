...
const calculateOEERow = (row) => {
  ...
  let pFgVal = v(27);
  if (pFgVal > 0) {
    setV(29, (pFgVal / (TEORI_BATCH[volKey] || 23076)).toFixed(2));
  } else {
    setV(29, '');
  }
...
const OEE_COLS_META = [
  ...
  { title: 'Bocor Sealing', width: 100, type: 'number' },
  { title: 'Bocor Logo', width: 90, type: 'number' },
  { title: 'Volume Kurang', width: 100, type: 'number' },
  { title: 'Volume Lebih', width: 90, type: 'number' },
...
export default function InputF() {
  ...
  const LS_OEE = 'F_DATA_OEE_L3', LS_DT = 'F_DATA_DT_L3', LS_IDS_OEE = 'F_IDS_OEE_L3', LS_IDS_DT = 'F_IDS_DT_L3';
  ...
  const handleFinishEdit = useCallback((rowIdx, colIdx, value, gridType, moveKey) => {
    setTimeout(() => {
      if (gridType === 'oee') {
        ...
        const maxC = DT_COLS_META.length - 1;
        if (moveKey === 'Enter' || moveKey === 'ArrowDown') nextR = Math.min(maxR, rowIdx + 1);
        else if (moveKey === 'ArrowUp') nextR = Math.max(0, rowIdx - 1);
        else if (moveKey === 'Tab' || moveKey === 'ArrowRight') nextC = Math.min(maxC, colIdx + 1);
        else if (moveKey === 'ArrowLeft') nextC = Math.max(0, colIdx - 1);
  ...
  const handleDownloadExcelOEE = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetchAllRejectF(line3User);
      if (res?.status === 'success' && Array.isArray(res.data)) {
        ...
        XLSX.writeFile(wb, `Semua_Data_OEE_Line3_ZoneF_${today}.xlsx`);
  ...
  const handleDownloadExcelDT = useCallback(async () => {
    ...
    try {
      const line3User = { ...(user || {}), line: "3" };
      const res = await fetchAllDowntimeF(line3User);
      if (res?.status === 'success' && Array.isArray(res.data)) {
        ...
        const today = new Date().toISOString().split('T')[0];
        XLSX.writeFile(wb, `Semua_Data_Downtime_Line3_ZoneF_${today}.xlsx`);
