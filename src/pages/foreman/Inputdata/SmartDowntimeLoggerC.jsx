    ...
    return (
        <div className="flex flex-col gap-1.5">
            ...
            <input 
                ...
                ${isDark ? 'bg-black/20 border-white/10 text-white focus:border-red-500' : 'bg-slate-50 border-slate-300 text-slate-800 focus:border-red-500'}
...
const HistoryTableDT = ({ data, refresh, onEdit, onDelete, isLoading, theme }) => {
    ...
    const stickyClasses = {
        ...
        shift: `sticky left-[90px] z-20 w-[60px] min-w-[60px] max-w-[60px] ${stBgTd}`,
        grup:  `sticky left-[150px] z-20 w-[60px] min-w-[60px] max-w-[60px] ${stBgTd}`,
        batch: `sticky left-[210px] z-20 w-[90px] min-w-[90px] max-w-[90px] ${stBgTd}`,
        ...
        <div className={`flex flex-col w-full h-full relative ${isFullView ? `bg-[#0B1120] rounded-2xl shadow-2xl` : `rounded-2xl shadow-2xl overflow-hidden`}`}>
            <div className={`p-3 flex justify-between items-center ${isDark ? 'bg-slate-800' : 'bg-slate-100'}`}>
                <div className="flex items-center gap-3">
                    <h3 className={`font-bold flex items-center gap-2 text-xs uppercase ${isDark ? 'text-white' : 'text-slate-800'}`}><Database size={14} className="text-red-500"/> Histori Downtime C</h3>
                ...
                <AnimatePresence>
                    {isLoading && (
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={`absolute inset-0 z-[100] flex flex-col items-center justify-center backdrop-blur-md ${isDark ? 'bg-[#1e293b]/60' : 'bg-white/60'}`}>
                        <Loader2 className={`animate-spin mb-2 ${isDark ? 'text-red-400' : 'text-red-600'}`} size={40}/>
                    ...
                    <table className="w-full border-collapse border-hidden">
                        <thead className="sticky top-0 z-40 shadow-xl">
                            <tr>
                                ...
                                <ThGroup rowSpan={2} className={`sticky left-[90px] z-50 w-[60px] min-w-[60px] max-w-[60px] ${stBgTh} ${isDark?'text-white':'text-slate-800'}`}>Shift</ThGroup>
                                <ThGroup rowSpan={2} className={`sticky left-[150px] z-50 w-[60px] min-w-[60px] max-w-[60px] ${stBgTh} ${isDark?'text-white':'text-slate-800'}`}>Grup</ThGroup>
                                <ThGroup rowSpan={2} className={`sticky left-[210px] z-50 w-[90px] min-w-[90px] max-w-[90px] ${stBgTh} ${isDark?'text-white':'text-slate-800'}`}>Batch</ThGroup>
                        ...
                        <tbody className={textMain}>
                            {(!processedData || processedData.length === 0) ? (<tr><td colSpan="15" className={`px-4 py-8 text-center italic text-xs ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Belum ada data downtime.</td></tr>) : (
                                processedData.map((row, idx) => {
                                    ...
                                    <tr key={idx} className={`transition-colors hover:bg-opacity-80 ${isDark ? 'bg-[#1e293b]/50 hover:bg-slate-800' : 'bg-white hover:bg-slate-50'}`}>
                                        ...
                                        <Td className={stickyClasses.grup}>{val(row, 4)}</Td>
                                        <Td className={stickyClasses.batch}><span className={isDark?'text-white font-black':'text-slate-900 font-black'}>{val(row, 5)}</span></Td>
                                        
                                        <Td className={isDark?'bg-indigo-900/10 text-blue-300':'bg-indigo-50/40 text-blue-700'}>{formatTime(row[6], row[7])}</Td>
                                        <Td className={isDark?'bg-indigo-900/10 text-blue-300':'bg-indigo-50/40 text-blue-700'}>{formatTime(row[8], row[9])}</Td>
                                        <Td className={isDark?'bg-red-900/10 text-red-400 font-bold':'bg-red-50/40 text-red-600 font-bold'}>{val(row, 10)}m</Td>
                                        
                                        <Td>
                                            <span className={`px-2 py-0.5 rounded text-[9px] border ${val(row, 11)==='Unplanned' ? (isDark?'bg-red-500/10 border-red-500/30 text-red-400':'bg-red-100 border-red-300 text-red-700') : (isDark?'bg-blue-500/10 border-blue-500/30 text-blue-400':'bg-blue-100 border-blue-300 text-blue-700')}`}>
                                                {val(row, 11)}
                                            </span>
                                        </Td>
                                        <Td>{val(row, 12)}</Td>
                                        <Td>{val(row, 13)}</Td>
                                        <Td>{val(row, 14)}</Td>
                                        <Td className="whitespace-normal max-w-[200px] text-left leading-tight italic opacity-80">{val(row, 15)}</Td>
                                        ...
                                        <Td className={stickyClasses.aksi}>
                                            <div className="flex justify-center items-center gap-1">
                                                <button type="button" onClick={() => { onEdit(row); setIsFullView(false); }} className={`p-1.5 rounded shadow-sm active:scale-95 ${isDark ? 'bg-yellow-500/10 text-yellow-400 hover:bg-yellow-500 hover:text-black' : 'bg-yellow-100 text-yellow-700 border hover:bg-yellow-400 hover:text-white'}`} title="Edit Data"><FileEdit size={14}/></button>
                                                <button type="button" onClick={() => onDelete(row)} className={`p-1.5 rounded shadow-sm active:scale-95 ${isDark ? 'bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-white' : 'bg-red-100 text-red-700 border hover:bg-red-500 hover:text-white'}`} title="Hapus Data"><Trash2 size={14}/></button>
                                            </div>
                                        </Td>
                                    </tr>
                                    )
...
const InputDowntimeC = () => {
    ...
    // --- STATE KERANJANG (MULTI-BATCH) ---
    // Di Zone C, tidak ada "Lot", jadi timeRows hanya butuh Batch + Jam
    ...
    const [timeRows, setTimeRows] = useState([ { id: Date.now(), no_batch: '', start_h: '', start_m: '', end_h: '', end_m: '', duration: 0 } ]);
    ...
    const loadData = async () => {
        try {
            ...
            if(res.status === 'success') {
                ...
                setOptions(prev => ({
                    ...prev,
                    rc: res.data['RC_C'] || [], proses: res.data['DT_Proses_C'] || [] 
                }));
            }
        } catch(e) {}
        try {
    ...
    const addTimeRow = () => {
        setTimeRows([...timeRows, { id: Date.now(), no_batch: '', start_h: '', start_m: '', end_h: '', end_m: '', duration: 0 }]);
    };
    ...
    const handleAddDraft = () => {
        ...
        timeRows.forEach(row => {
            if(!row.no_batch || !row.start_h || !row.end_h) {
        ...
        if(hasError) {
            toast.error("Setiap baris harus memiliki No Batch, Jam Mulai, dan Selesai yang lengkap!"); return;
        }

        setDrafts([...drafts, ...newDrafts]);
        toast.success(`${newDrafts.length} Data digandakan ke Daftar!`, { icon: '🛒' });
        
        setTimeRows([ { id: Date.now(), no_batch: '', start_h: '', start_m: '', end_h: '', end_m: '', duration: 0 } ]);
    };
    ...
    const handleDeleteClick = async (rowData) => {
        if (window.confirm(`⚠️ PERINGATAN: Apakah Anda yakin ingin menghapus permanen Downtime Batch ${rowData[5]}?`)) {
            ...
            try {
                ...
                const res = await submitOEEData({ action: 'delete_downtime_c', data: finalData }, user);
                if(res.status === 'success') {
    ...
    const handleSubmit = async () => {
        if (isEditing) {
            ...
            try {
                ...
                const res = await submitOEEData({ action: 'update_downtime_c', data: payload }, user);
                setLoading(false);
                if(res.status === 'success') {
            ...
            setLoading(true);
            try {
                const res = await submitOEEData({ action: 'submit_bulk_downtime_c', data: drafts }, user);
                setLoading(false);
                if(res.status === 'success') {
    ...
    const handleEditClick = (rowData) => {
        ...
        setHeaderData({ 
            ...
            plan_unplan: rowData[11], root_cause: rowData[12], proses: rowData[13], unit: rowData[14], kasus: rowData[15]
        });
        
        // Mode edit hanya memasukkan 1 baris waktu
        setTimeRows([{
            id: Date.now(), no_batch: rowData[5], start_h: rowData[6], start_m: rowData[7], end_h: rowData[8], end_m: rowData[9], duration: rowData[10]
        }]);
            ...
            <div className={`backdrop-blur-md border-b px-6 py-4 sticky top-0 z-50 transition-colors duration-300 ${bgHeader} ${isEditing ? 'border-b-2 border-yellow-500' : ''}`}>
                <div className="max-w-6xl mx-auto flex justify-between items-center">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-xl font-black tracking-tight flex items-center gap-2">INPUT DOWNTIME <span className="text-red-500">ZONE C</span></h1>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        ...
                        <div className={`p-6 rounded-2xl border shadow-xl transition-colors duration-300 ${theme === 'dark' ? 'bg-indigo-900/10 border-indigo-500/20' : 'bg-indigo-50 border-indigo-200'}`}>
                            ...
                            {/* JUDUL KOLOM UNTUK MULTI INPUT (ZONE C TANPA LOT) */}
                            <div className={`hidden md:flex items-center gap-3 px-3 mb-2 text-[10px] font-bold uppercase tracking-wider ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>
                                ...
                                <div className="flex-1 grid grid-cols-10 gap-3">
                                    <div className="col-span-3 text-center">No Batch</div>
                                    ...
                                    <div key={row.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${theme === 'dark' ? 'bg-[#0f172a] border-white/5 hover:border-indigo-500/50' : 'bg-white border-slate-300 hover:border-indigo-400'}`}>
                                        ...
                                        <div className="flex-1 grid grid-cols-1 md:grid-cols-10 gap-3">
                                            <div className="col-span-1 md:col-span-3">
                                                <input type="text" value={row.no_batch} onChange={(e) => handleTimeRowChange(row.id, 'no_batch', e.target.value)} placeholder="No Batch" className={`w-full px-3 py-2 rounded-lg border outline-none font-bold text-xs uppercase text-center ${theme === 'dark' ? 'bg-black/40 border-slate-700 text-white' : 'bg-slate-100 border-slate-300 text-slate-800'}`}/>
                                            </div>
                                            ...
                                            <div className="col-span-1 md:col-span-1 flex items-center justify-between">
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        {isEditing ? (
                            ...
                            <>
                                <button type="button" onClick={handleAddDraft} className={`w-full py-5 rounded-2xl border-2 border-dashed font-black text-lg flex items-center justify-center gap-2 transition-all active:scale-95 ${theme === 'dark' ? 'border-red-500/50 hover:border-red-400 hover:bg-red-500/10 text-red-400' : 'border-red-400 hover:border-red-600 hover:bg-red-50 text-red-600'}`}>
                                    <Plus size={24}/> GANDAKAN KE DAFTAR
                                </button>
                                        ...
                                        {drafts.length === 0 ? (
                                            ...
                                            drafts.map((d) => (
                                                <div key={d.draftId} className={`p-3 rounded-xl border flex justify-between items-center group ${theme==='dark'?'bg-slate-800 border-slate-700':'bg-white border-slate-300'}`}>
                                                    <div>
                                                        <p className={`font-bold text-xs ${theme==='dark'?'text-white':'text-slate-800'}`}>Batch <span className="text-red-500">{d.no_batch}</span></p>
