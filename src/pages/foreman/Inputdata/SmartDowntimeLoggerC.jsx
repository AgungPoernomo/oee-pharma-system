        ...
        <div className="mt-12 bg-[#1e293b] rounded-2xl border border-white/5 shadow-xl overflow-hidden">
            <div className="p-4 border-b border-white/5 flex justify-between items-center bg-gradient-to-r from-red-900/20 to-slate-900">
                <h3 className="font-bold text-white flex items-center gap-2">
                    <Database size={18} className="text-red-400"/> LIVE MONITORING
                </h3>
                        ...
                        {(!data || !Array.isArray(data) || data.length === 0) ? (
                            ...
                            data.map((row, idx) => (
                                <tr key={idx} className="hover:bg-white/5 transition-colors">
                                    ...
                                    <Td className="text-red-400 font-bold">{val(row, 10)} m</Td>
                                    <Td><span className={`px-2 py-1 rounded text-[10px] border ${val(row, 11)==='Unplanned'?'bg-red-500/10 border-red-500/30 text-red-400':'bg-blue-500/10 border-blue-500/30 text-blue-400'}`}>{val(row, 11)}</span></Td>
                                    <Td>{val(row, 12)}</Td> <Td>{val(row, 13)}</Td> <Td>{val(row, 14)}</Td> 
                                    <Td className="italic text-slate-400 truncate max-w-[150px]">{val(row, 15)}</Td>
...
const InputDowntimeC = () => {
    ...
    const initialHeader = { tanggal: new Date().toISOString().split('T')[0], shift: '', group: '', no_batch: '' };
    ...
    const [drafts, setDrafts] = useState([]); // State untuk menyimpan daftar draf downtime (Bulk)
    ...
    const handleAddDraft = () => {
        // Validasi
        if(!headerData.no_batch || !headerData.shift || !headerData.group) {
            toast.error("Data Umum (Shift, Group, Batch) wajib diisi dahulu!"); return;
        }
    ...
    const handleSubmit = async () => {
        if (isEditing) {
            ...
            setLoading(true);
            try {
                // Di sini kita menghantar array drafts ke backend (action baru)
                ...
                if(res.status === 'success') {
                    ...
                    setHeaderData(initialHeader); // Reset header jika perlu
                    loadData();
                } else { toast.error(res.message); }
            } catch(e) { setLoading(false); toast.error("Error koneksi"); }
        }
    ...
    const handleEditClick = (rowData) => {
        ...
        setHeaderData({ tanggal: dateVal, shift: rowData[3], group: rowData[4], no_batch: rowData[5] });
        setDetailData({
            start_h: rowData[6], start_m: rowData[7], end_h: rowData[8], end_m: rowData[9],
            duration: rowData[10], plan_unplan: rowData[11], root_cause: rowData[12],
            proses: rowData[13], unit: rowData[14], kasus: rowData[15]
        });
            ...
            <div className="max-w-4xl mx-auto px-4 mt-6">
                ...
                <section>
                    ...
                    <div className="bg-[#1e293b] p-6 rounded-2xl border border-white/5 shadow-lg space-y-4">
                        <DropdownGroup label="Kategori (Planned/Unplanned)" name="plan_unplan" value={detailData.plan_unplan} options={['Unplanned', 'Planned']} onChange={handleDetailChange} required/>
                        ...
                        <div className="flex flex-col gap-1.5">
                            ...
                            <textarea name="kasus" value={detailData.kasus} onChange={handleDetailChange} rows="2" placeholder="Jelaskan detail masalah..." className="w-full px-4 py-3 rounded-xl bg-black/20 border border-white/10 text-white focus:border-red-500 outline-none text-sm"></textarea>
                        </div>
                    </div>
                </section>

                {/* ACTION BUTTON (TAMBAH KE DRAF ATAU UPDATE SINGLE) */}
                <div className="mt-6">
                    {isEditing ? (
                        ...
                        <button type="button" onClick={handleAddDraft} className="w-full py-4 rounded-xl border-2 border-dashed border-red-500/50 hover:border-red-500 hover:bg-red-500/10 text-red-400 font-bold text-lg flex items-center justify-center gap-2 transition-all">
                            <Plus size={24}/> TAMBAH KE DAFTAR
                        </button>
                    )}
                </div>

                {/* KERANJANG DRAF (HANYA MUNCUL JIKA BUKAN MODE EDIT DAN ADA DRAF) */}
